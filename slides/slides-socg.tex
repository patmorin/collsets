...
\begin{frame}
   ...
   \begin{itemize}[<+->]
      ...
      \begin{center}
        ...
        \only<9->{\includegraphics{figs/cycle-to-curve-6}}
      \end{center}
   \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}[<+->]
    ...
    \begin{center}
      \begin{tabular}{cc}
        \includegraphics[width=.3\textwidth]{figs/cycle-to-curve-1} &
        \includegraphics[width=.3\textwidth]{figs/cycle-to-curve-2}
      \end{tabular}
  \end{center}
  \end{itemize}
...
\begin{frame}
  ...
  \begin{itemize}
     \item[]\textbf{Theorem:} If $G$ has maximum degree $\Delta$ and $\dual{G}$ has a cycle $C$ of length $\ell$, then $\dual{G}$ has a cycle $C'$ that \caresses\ $\kappa=\Omega(\ell/\Delta^4)$ faces.
  \end{itemize}
\end{frame}
...
% \begin{frame}
%   \frametitle{Each Leaf $\kappa$aresses a Face}
% 
%    \noindent\textbf{Lemma:} 
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \only<1->{\item[$\checkmark$] Each leaf of $T_i$ contains a \caressed\ face} (skipped---easy)
      \item Each $T_i$ has $\Omega(\ell/\Delta)$ nodes
    \end{itemize}
    \item We are done if either $T_i$ has many leaves, so each $T_i$ must have few leaves
    \begin{itemize}
      \item[$\therefore$] each $T_i$ has many \emph{bad nodes} \\ (degree-2 and containing no caressed faces)
    \end{itemize}
    \item \alert<2->{With enough bad nodes, we can perform a \emph{surgery} that increases the number of leaves in $T_0$.}
  \end{itemize}
\end{frame}
...
\begin{frame}
  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \only<2->{\item[$\checkmark$] Each $T_i$ has $\Omega(\ell/\Delta)$ nodes} (skipped---discharging argument)
    \end{itemize}
    \item We are done if either $T_i$ has many leaves, so each $T_i$ must have few leaves
    \begin{itemize}
      \item[$\therefore$] each $T_i$ has many \emph{bad nodes} \\ (degree-2 and containing no caressed faces)
...
\begin{frame}
  ...
  \begin{itemize}
    \item[]\textbf{Theorem:} If $\dual{G}$ has a cycle of lenth $\ell$, then $\dual{G}$ has a cycle $C'$ that caresses $\Omega(\ell/\Delta^4)$ faces. \\[1em]

    \item[]\textbf{Consequence:} Every $n$-vertex planar graph of maximum degree $\Delta$ has a free set of size $\Omega(n^{0.8}/\Delta^4)$. \\[1em]

    \item[]\textbf{Open Problem:} Eliminate the dependence on $\Delta$.\\[2em]
    
    \item<2->[] \centerline{\Huge \color{red}T\color{orange}h\color{yellow}a\color{green}n\color{blue}k\color{purple}s\color{violet}!}
  \end{itemize}
\end{frame}
