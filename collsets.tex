...
We also note the following useful corollary that follows immediately from \lemref{many-caressed-or-high-degree} and the identity $\tau_u=\rho_u+\kappa_u$.
\begin{cor}\corlabel{many-caressed-or-high-degree}
...
\begin{proof}[Proof of \lemref{many-caressed-or-high-degree}]
  ...
  \begin{enumerate}
     ...
     Otherwise $R$ contains at least one face of $\dual{T}$ and \lemref{one-caressed}  ensures that $R$ contains at least one caressed face $f$.  We move the charge from $P$ onto $f$ and we are done.
...
\begin{proof}
   ...
   \begin{align*}
    ...
    \end{equation}
    If $\kappa_i \ge \tau_i/6$, then the proof is complete since $\tau_i/6>\tau_i/24$.  On the other hand, if $\kappa_i \le \tau_i/6$ then, by \lemref{few-caressed-implies-many-nodes}, $n_i \ge \tau_i/8$. Combining this with \eqref{blech}  gives
    \[
...
\begin{proof}
   ...
   boundary of $u$.  There are a few cases to rule out (see \figref{common-face}):
   \begin{figure}
       \begin{center}
           \centering{
           \begin{tabular}{cc}
           ...
           \caption{Cases in the proof of \lemref{common-face}}
           \figlabel{common-face}
       \end{center}
   \end{figure}
   \begin{enumerate}
...
\begin{proof}
   By \lemref{common-face}, there is one face $f$ of $\dual{T}$ contained in $u$ that contains both the first and last edge of $P$ and there is one face $g$ of $\dual{T}$ contained in $w$ that contains both the first and last edge of $P$.  By \lemref{one-shared-edge}, $f$ and $g$ have only one edge in common so the first and last edge of $P$ must be the same, i.e., $P$ consists of a single edge.
\end{proof}
...
\begin{proof}
  Without loss of generality, let $i=0$. From \obsref{degree-touched}, we know that $\tau_0\ge \ell/\Delta$.  Therefore, $\kappa_0\le\kappa\le \alpha\ell/\Delta \le \tau_0/6$ so, by \lemref{few-caressed-implies-many-nodes}, $n_0 \ge
  \tau_0/8$.

  By \lemref{most-nodes-are-bad}, if $b_0 < (1-24\alpha)n_0$, then
  \[
  ...
  \]
  Now, $A_1$ contains leaves of $T_0$ and, by \lemref{one-caressed-leaf}, each leaf of $T_0$ contains a caressed face.  Therefore $|A_1|\le\kappa$.
  ...
  \begin{align*}
   \sum_{w\in A'}\Delta \tau_w
    & \le  \sum_{w\in A'}(3\Delta\kappa_w + 2\delta_w) & \text{(by \corref{many-caressed-or-high-degree})} \\
...
We say that a node $u$ is \emph{really really bad} if all the nodes in $N(u)$ are really bad.  (Note that this implies that $u$ is bad.)  The following lemma extends \lemref{lots-of-really-bad} to really really bad nodes:

\begin{lem}\lemlabel{lots-of-really-really-bad}
  For each $i\in\{0,1\}$ and each $0<\alpha < 1/24$,
  if $T$ has maximum degree $\Delta$, $C$ has length $\ell$, and the number
...
We summarize the situation so far.  By \lemref{cycle-to-curve}, finding a large collinear set is equivalent to finding a cycle in $\dual{T}$ that caresses many faces.  By existing results on the circumference of cubic triconnected graphs, $\dual{T}$ has a cycle $C_0$ of length $\ell=\Omega(n^{\alpha})$ for some $\alpha > 0.8$.  Thus we assume that $\dual{T}$ has a cycle $C_0$ of length $\ell$ and we want to show the existence of a cycle $C$ that caresses $\Omega(\ell/\Delta^4)$ faces.

Because each face of $\dual{T}$ has at most $\Delta$ edges, $C_0$ touches $\Omega(\ell/\Delta)$ faces (\obsref{many-touched}).  To complete the proof of \thmref{main} we must deal with the situation where $C_0$ caresses $o(\ell/\Delta^4)$ faces and therefore each of $T_0$ and $T_1$ has $o(\ell/\Delta^4)$ leaves (\lemref{one-caressed-leaf}), $\Omega(\ell/\Delta)$ nodes (\lemref{few-caressed-implies-many-nodes}), and the fraction of really really bad nodes in $T_0$ and $T_1$ is $1-O(1/\Delta^2)$ (\lemref{lots-of-really-i-bad}).  \figref{few-caressed-ii} illustrates an extreme example of this.  To handle cases like these, the only option is to perform surgery on the cycle $C$ to increase the number of caressed faces. To complete the proof we will show that such a surgery is always possible.
...
$O(\epsilon n_0/\Delta)$
components.  To see why this is so, let $L$ be the set of leaves in $T_0$
and let $S$ be the set of non-leaf nodes in $T_0$ that are not really
really bad.  Observe that it is sufficient to upper bound the number,
$k$, of components in $T_0-S$.
