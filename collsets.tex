...
\begin{proof}
   ...
   \begin{compactenum}[(Post1)]
    ...
    The discharging procedure is made up of two routines, an \emph{initialization procedure} and a \emph{recursive procedure}.  The recursive procedure takes a pair of inputs $(P,c)$ where $P$ is a chord path that partitions $u$ into two parts $L$ and $R$ and $c\in\{0,1,2\}$ is a charge that we think of as sitting on $P$.  The pair $(P,c)$ must satisfy the following conditions:
   \begin{compactenum}[(Pre1)]
        \item Each face of $\dual{T}$ in $L$ that shares an edge with $P$
        is pinched.
        \item If $c=2$ then $P$ is contained in the boundary of a single face of $\dual{T}$ that is contained in $L$.
    \end{compactenum}
    The procedure guarantees that, after its completion, the faces contained in $R$ satisfy (Post1)--(Post3).

    Before defining the recursive procedure itself, we will show how it is used by the iniitalization procedure.  This initialization procedure takes an arbitrary pinched face $f$ contained in $u$.  Since $f$ is pinched, it has $r\ge 2$ chord paths $P_1,\ldots,P_r$ on its boundary and $u\setminus f$ has components $R_1,\ldots,R_r$ where $P_i$ separates $R_i$ from $f$, for each $i\in\{1,\ldots,r\}$.  This initialization procedure guarantees that, after it runs, all the faces and chord paths in $u\setminus R_1$ satisfy (Post1)--(Post3).  The initialization procedure does not have any affect on $R_1$.

    The initialization procedure works as follows: Since $f$ is pinched it has a charge of 1 so we move the charge from $f$ onto $P_2$ and apply the recursive procedure to $P_2$, with a charge of $c=1$, with $L=L_2=u\setminus R_2$.  Since $f$ is a pinched face, this satisifies the (Pre1) and since $c=1$ this satisfies (Pre2).  Once the recursive procedure is complete conditions (Post1)--(Post2) are satisfied for all faces in $f\cup R_2$.

    Next, we apply the recursive procedure on each of $P_3,\ldots,P_r$ with a charge of $c=0$.  When calling the procedure on $P_i$, $L=L_i$ is the component of $u\setminus P$ that contains $f$ and $R=R_i=u\setminus L_i$.  Since $f$ is a pinched face, this satisifies (Pre1) and since $c=0$ this satisfies (Pre2).  Once the recursive procedure is complete conditions (Post1)--(Post2) are satisfied for all faces in $f\cup R_i$.
    ...
    Next we describe the recursive discharging procedure that takes $(P,c)$ satisifying (Pre1) and (Pre2) and moves charges in $R$ so that they satisfy (Post1)--(Post3).  There are several cases to consider (see \figref{discharging}):
	\begin{figure}
		\begin{center}
		\begin{tabular}{cc}
  ...
  \begin{enumerate}
	...
	and $X_1,\ldots,X_k$, where $k\ge 1$, $A$ and $B$ each contain an endpoint
    ...
    The recursive call on $A$ satisfies (Pre1) because the path $P_A$ used in this recursive call is contained in the boundary of $f$ and $P$. In particular each face of $\dual{T}$ contained in $u\setminus A$ that is incident to $P_A$ is either a face of $L$ incident to $P$ or is the face $f$.  The latter faces are pinched by (Pre1) and $f$ is pinched by definition.  The recursive call on $A$ also satisifies (Pre2) since, in this call, $c\le 1$.  The same argument shows that the recursive call on $B$ satisfies (Pre1) and (Pre2).

    When recursing on $X_1$ we move the unit of	charge from $f$ to $P_1$.  When recursing on $X_2,\ldots,X_k$ we use no charge on $P_2,\ldots,P_k$.  Each recursive call satisifes (Pre1) because $f$ is pinched and satisfies (Pre2) because the value of $c$ in the recursive call is at most 1.

	\item $f$ contains exactly one endpoint of $P$.  In this case, $R\setminus f$ has one connected component $A$ that contains an endpoint of $P$ and one or more connected components $X_1,\ldots,X_k$ where each $X_i$ has a chord path $P_i$ on the boundary of $f$.	The path $P$ has a charge $c\le 2$.  When recursing on $X_1$ we assign all of $P$'s charge to the chord path $P_1$, which is contained in the single pinched face $f$.
    This recursive call satisfies (Pre1) because $f$ is pinched and it satisfies (Pre2) because $P_1$ is entirely contained in $f$.

    When recursing on each of $X_2,\ldots,X_k$ we recursive with the value $c=0$.  These recursive calls also satisfy (Pre1) and (Pre2).

	When recursing on $A$ we move the single unit of charge from $f$
	to the chord path of $A$.  This satisfies (Pre1) and (Pre2) for the same reasons outlined in the previous case.

    \item $f$ contains both endpoints of $P$.  We claim that, in this case, $P$ must be on the boundary of more than one pinched face in $L$, otherwise $P$ would be a keeper path. To see this, observe that the face $f$ contains both the first edge $e_1$ and last edge $e_2$ of $P$. If $e_1=e_2$ because $P$ is a single edge, then it is certainly a keeper, which is not possible. Otherwise, by \lemref{one-shared-edge}, $e_1$ and $e_2$ are on the boundary of two different faces in $L$.
    % By (Pre1), both of these faces are pinched by $C$.

    Therefore, by (Pre2) $P$ has at most one unit of charge assigned to it. Now, $R\setminus f$ has one or more connected components $X_1,\ldots,X_k$ sharing chord paths $P_1,\ldots,P_k$ with $f$ on which we recurse.  When recursing on $X_1$ we move the charge from $P$ and the charge from $f$ to $P_1$.  This satisfies (Pre1) because $f$ is pinched and satisfies (Pre2) because $P_1$ is contained on the boundary of $f$.

    When recursing on the remaining $X_i$, $i\in\{2,\ldots,k\}$ we assign no charge to $P_i$.  These recursive calls satisfy (Pre1) because $f$ is pinched and satisfy (Pre2) because, in each call $c=0$.
  \end{enumerate}
  ...
  Now, $R$ still contains one or more pinched faces  $f_1,\ldots,f_k$, such that each $f_i$ shares part of a chord path $P_i$ with $g$.  Consider one such $f_i$ and observe that $u\setminus f_i$ has $r_i\ge 2$ components $R_{i,1},\ldots,R_{i,r_i}$. For each $j\in\{1,\ldots,r_i\}$, let $P_{i,j}$ be the chord path that separates $R_{i,j}$ from $f_i$.  We use the convention that $P_{i,1}=P_i$ so that $R_{i,1}$ is the component of $u\setminus f_i$ that contains $g$.   On each such face $f_i$, we run the initialization procedure and this reorganizes the charges in $u\setminus R_{i,1}$ so that they satisfy (Post1)--(Post3).  Doing this for each $i\in\{1,\ldots,k\}$ completes the description of the discharging procedure.
\end{enumerate}
