...
\author{Vida Dujmovi\'c}{School of Computer Science and Electrical Engineering, University of Ottawa}{vida.dujmovic@uottawa.ca}{https://orcid.org/0000-0001-7250-0600}{Partly funded by NSERC and the Government of Ontario.}
...
\begin{abstract}
   ...
   has a non-crossing straight-line drawing in which some \emph{collinear set}
   ...
   $|X|=|S|$, $T$ has a non-crossing straight-line drawing in which the
...
$S$ are all collinear.  A \emph{plane graph} is a planar graph $G$ along with
a particular non-crossing drawing of $G$.  The \emph{dual} $\dual{G}$
of a plane graph $G$ is the graph whose vertex set $V(\dual{G})$ is
the set of faces in $G$ and in which $fg\in E(\dual{G})$ if and only
if the faces $f$ and $g$ of $G$ have at least one edge in common.
...
\begin{cor}\corlabel{main}
  There exists a constant $\alpha > 0.8$ such that every $n$-vertex
  planar graph of maximum degree $\Delta$ contains a collinear set of
  size $\Omega(n^{\alpha}/\Delta^4)$.
\end{cor}
...
%For example \corref{main} implies that every bounded-degree
%$n$-vertex geometric planar graph can be untangled while keeping
%$\Omega(n^{0.4})$ vertices fixed. The previous best result for
%untangling any $n$-vertex geometric planar graphs was $\Omega(n^{0.25})$
%\cite{bose.dujmovic.ea:polynomial}.  One of the key steps in the proof
%of the $\Omega(n^{0.25})$ involves applying Dilweth's Theorem twice
...
Due to the 500 line submission limit, the 
proofs of \obsref{degree-touched} and Lemmas \ref{lem:one-caressed}, \ref{lem:common-face} and \ref{lem:lots-of-really-bad} are deferred to \appref{missing-proofs}.
...
a closed walk in the dual $\dual{T}$ of $T$.  The properness of the original curve and the fact that each face of $T$ is a triangle ensures that $f_i\neq
...
\end{center}
	  \caption{Faces of $\dual{T}$ that are pinched and caressed by $C$. $C$ is bold, caressed faces are teal, pinched faces are pink, and untouched faces are unshaded.}
...
\begin{proof}
   ...
   We now describe the discharging procedure, which is recursive and take as
...
\section{Proofs of \obsref{degree-touched} and Lemmas \ref{lem:one-caressed}, \ref{lem:common-face} and \ref{lem:lots-of-really-bad}}
\applabel{missing-proofs}


\begin{proof}[Proof of \lemref{one-shared-edge}]
...
\begin{proof}[Proof of \lemref{one-caressed}]
   The proof is by
   induction on the number, $t$, of faces of $\dual{T}$ contained in $R$.
   If $t=1$, then $R$ is a face of $\dual{T}$ and it is caressed by $C$.
...
\begin{proof}[Proof of \obsref{degree-touched}]
