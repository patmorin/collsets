...
\begin{abstract}
   ...
   size $\Omega(n^{0.8}/\Delta^4)$.  Such collinear sets have numerous
...
Recently, Dujmovi\'c \etal\ \cite{dujmovic.frati.ea:every}, have
shown that every collinear set is \emph{free}. That is, for any planar
graph $G$, any collinear set $S\subseteq V(G)$, and any set $X\subset\R^2$,
...
related areas \cite{dujmovic:utility}.  The consequences of \corref{main}
...
embedding from the embedding of $\tilde{H}$.
This embedding partitions the edges of $\tilde{H}'$ into three sets:
\begin{enumerate}
...
\begin{proof}
  ...
  \begin{enumerate}
	...
	$A$'s chord path.  When recursing on $B$ we take the second unit
	of charge from $P$ (if needed) and place it on $B$'s chord path.
	When recursing on $X_1$ we move the unit of charge from $f$
	to $P_1$.  When recursing on $X_2,\ldots,X_k$ we use no charge
	on $P_2,\ldots,P_k$.
...
is tight. Make a figure to show this?}
...
each $i\in\{0,1\}$,
\begin{enumerate}
  \item $\tau_i$, $\rho_i$, and $\kappa_i$ refer the total numbers of faces contained in nodes of $T_i$ that touched, pinched, and caressed by $C$, respectively; 
...
%  and caressed by $C$ and let $n_i$ denote the number of nodes of $T_i$.
  If $\kappa_i \le \tau_i/4$ then $n_i\ge \tau_i/8$.
\end{lem}
...
\begin{proof}
   ...
   \begin{align*}
    ...
    \end{equation}
    If $\kappa_i \ge \tau_i/4$, then the proof is complete.  On the other hand, if $\kappa_i \le \tau_i/4$ then, by \lemref{few-caressed-implies-many-nodes}, $n_i \ge \tau_i/8$. Combining this with \eqref{blech}  gives
    \[
...
\begin{proof}
  ...
  $\kappa_0\le\kappa\le \alpha\ell/\Delta^2 \le \tau_0/4$
...
\begin{enumerate}
   ...
   \item a path $b_1,\ldots,b_s$, $s\ge 1$, of really bad nodes in $T_1$, each sharing an eedge of $C_b$ with $u$.
\end{enumerate}
\begin{figure}
   \begin{center}
...
$b_0$, and $b_{s+1}$ are really bad because the are adjacent to at least
...
\end{figure}
     ...
     \begin{enumerate}
       ...
       We claim that $z_1$ contains of a single face of $\dual{T}$.  By
...
$\ell/(2\Delta)\in\Omega(\ell/\Delta)\subset\Omega(\ell/\Delta^3)$ and we
