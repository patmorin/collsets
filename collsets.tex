...
\begin{proof}
   ...
   \begin{enumerate}
       ...
       this case, $P$ is a chord path and, by \lemref{chord-path}
       ...
       The path $P$ is not a keeper, other it would have split $u$ into
       two parts.  Therefore, it must be the case that $P$ contains
...
these nodes interact.  For a node $u$ of $T_i$, we define $N(u)$ as
the set of nodes (in $T_0$ and $T_1$) that share an edge of $\dual{T}$
with $u$.  
...
\begin{lem}\lemlabel{one-shared-edge}
   If $u$ is a really bad node, then $u$ shares a most one edge of $C$
   with each node in $N(u)$.
\end{lem}

\begin{proof}
   Suppose two bad nodes $u$ and $w$ share two edges $e_1$ and $e_2$
...
We say that a node $u$ is \emph{really bad} if every node in
$\{u\}\cup N(u)$ is bad.
