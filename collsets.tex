...
\begin{lem}\lemlabel{many-good-or-high-degree}
...
\begin{proof}
   ...
   (see \figref{discharging}):
	\begin{figure}
		...
		\end{tabular}
		\end{center}
		\caption{Discharging steps in the proof of \lemref{many-good-or-high-degree}.}
...
\end{equation}
From \lemref{many-good-or-high-degree}, we have 
\[
...
%Observe that, if $T$ has maximum degree $\Delta$, then each face of
%$\dual{T}$ has at most $\Delta$ edges on its boundary.  Since $C$
%is a non-trivial cycle, it can contain at most $\Delta-1$ edges from
%any face of $\dual{T}$.  Each edge of $C$ is on the boundary of two
%faces of $\dual{T}$, contained in the interior of $C$ and one in the
%exterior of $C$.  Therefore, if $C$ has length $\ell$, then it touches
%at least $\tau\ge 2\ell/(\Delta-1)$ faces of $\dual{T}$. At least
%$\ell/(\Delta-1)$ of the faces $C$ touches are in the interior of $C$
%and at least $\ell/(\Delta-1)$ are in the exterior of $C$. 
%
%For each $i\in\{0,1\}$, let $\tau_i$, $\rho_i$ and $\kappa_i$ denote
%the number of touched, pinched, and caressed faces of $\dual{T}$
%that are contained in nodes of $T_i$.  Let $m_{2,i}$ denote the number
%of degree-2 nodes in $T_i$. The preceding paragraph argues
%that $\tau_i\ge \ell/(\Delta-1)$ and the same argument used in \lemref{many-caressed-or-many-degree-2} allows us to conclude that
%\[
%    11\kappa_i + 4m_{2,i} \ge \tau_i \ge \ell/(\Delta-1) \enspace .
%\]
%
%We say that a node of $T_i$ is \emph{bad} if it has degree-2 and contains
%no face of $\dual{T}$ that is caressed by $C$.
%Let $m_{2,i}'$ denote the number bad nodes in $T_i$.
%Thus far, we know:
%\begin{enumerate}
%   \item If $m_{2,i}\le \tau_i/8$, then $\kappa_i \ge \tau_i/22 \ge \ell/(22(\Delta-1))$ and, by \lemref{cycle-to-curve} $v(T)\ge \ell/(88\Delta-1)$.
%
%   \item Otherwise, if $m_{2,i}' \le (1-\epsilon)m_2$, then 
%\[
%    m_{2,i}-m_{2,i'} \ge \epsilon m_2 \ge \epsilon\tau_i/8 \ge (\epsilon/8)\ell/(\Delta-1)$ \enspace .
%\]
%Therefore, by \lemref{cycle-to-curve} $v(T)\ge \epsilon\ell/(4(\Delta-1))$
%
%    \item If $T_i$, contains $\epsilon m_2$\epsilon\ell/(\Delta-1)$ nodes of degree 3 or more, then $T_i$ contains at least $\epsilon\ell/(\Delta-1)$ leaves.  In this case \lemref{one-caressed-leaf} and \lemref{cycle-to-curve} imply that $v(T)\ge \epsilon\ell/(4(\Delta-1))$.
%\end{enumerate}
%
%Therefore, for any $\epsilon \le 1/22$, we conclude that
%$v(T)\ge\epsilon\ell/(4(\Delta-1))$ or that each of $T_0$ and $T_1$
%contain at least $(1/8-\epsilon)\ell/(\Delta-1)$ bad nodes.
%

We say that a node of $T_i$ is \emph{bad} if it has degree-2 and contains
no face of $\dual{T}$ that is caressed by $C$.

\begin{lem}\lemlabel{all-bad}
...
these nodes interact.  The following lemma shows that a node $u$ in $T_0$
and $w$ in $T_1$ share at most one edge of $C$.
...
\begin{proof}
   Suppose $u$ and $w$ share two edges $e_1$ and $e_2$
   of $C$.  Then, by \lemref{one-face}, there is a common face $f_u$
   in $u$ that contains $e_1$ and $e_2$, Similarly, there is a common
   face $f_w$ contained in $w$ that contains both $e_1$ and $e_2$.
   This means that $e_1$ and $e_2$ form an edge cutset of $\dual{T}$.
   This is not possible because $\dual{T}$ is 3-connected and 3-regular.
\end{proof}
...
\begin{lem}
  If $T$ has maximum degree $\Delta$ and, for each $i\in\{0,1\}$, $b_i > (1-\epsilon)n_i$, then $T_0$ has $(1-\alpha\epsilon)n_i/\Delta$ really bad nodes.
\end{lem}

\begin{proof}
  Let $A_1$ be the set of nodes in $T_1$ that are not bad.
  Each node $w$ in $T_1$ is contained in $N(u)$ for at most $\Delta$
  nodes $u$ in $T_0$.  Therefore, each node $w$ in $T_1$ that is not
  bad eliminates at most $\Delta\tau_w$ nodes from $T_0$.  
  Therefore,
  the number of nodes in $T_0$ that are not really bad is at most
  \[
     \sum_{w\in V(T_1)} \Delta\tau_w \le \sum_{w\in V(T_1)} \Delta\tau_w
  \]
\end{proof}


If
  If $\kappa < \epsilon \ell/(\Delta-1)^2$, then the number of bad nodes in $T_0$ is at least $(1-\epsilon)m$.
\end{lem}

\begin{proof}
  Each non-bad kills at most $\Delta+1$ node.
\end{proof}


%\begin{lem}
%  If $T$ has maximum degree $\Delta$ then, for any really bad node $u$,
%  $|N(u)|\le\Delta + 2$.
%\end{lem}
%
%\begin{proof}
%  Suppose, without loss of generality, that $u$ is a node of $T_0$.  Since $u$ has degree 2, $N(u)$ contains at most 2 nodes of $T_0$.  Every other node in $N(u)$ shares at least one edge of $C$ with $u$.  However, \lemref{same-face} implies that all of these edges of $C$ are contained in a common face $f$ of $\dual{T}$. The number of edges on the boundary of $f$ is at most $\Delta$.
%\end{proof}
...
%
%\begin{lem}
%   If $u$ a really bad node, then every edge of $E(C)$ on the boundary
%   of $u$ is incident to exactly two (bad) nodes, one of which is $u$.
%\end{lem}
%
%\begin{proof}
%   Let $C_1$ and $C_2$ be the two components of $C\cap u$.  Then, by \lemref{blech}, there is a single face $f$ contained in $u$ that contains $C_1$ and $C_2$.  Since $\dual{T}$ is 3-regular, this implies that $u$ 
%\end{proof}
%
%
%$u$ is a face of $\dual{T}$.
%\end{lem}
%
%
%\begin{lem}\lemlabel{u-is-face}
%   If $u$ is a really bad node, then $u$ is a face of $\dual{T}$.
%\end{lem}
%
%
%
%\begin{lem}\lemlabel{u-is-face}
%   If $u$ is a really bad node, then $u$ is a face of $H$.
%\end{lem}
%
%\begin{proof}
%A little bit of cases.
%%   First observe that, since $u$ has degree-2, $C\cap u$ has exactly
%%   two connected components $C_1$ and and $C_2$. Thus $u$'s boundary
%%   consists of $C_1$, $C_2$ and two chord paths $P_1$ and $P_2$.
%%
%%   Next, note that no edge in the interior of $u$ is incident to $C_1$ or $C_2$.  If there were such an edge $e$ incident to a vertex of $C_1$, say, and this edge began a path $P$ in the interior of $u$ leading to a vertex of $P_1$ or $P_2$, then $P$ along with a subpath of $P_1$ or $P_2$ would create a chord path in $u$ whose endpoints are both $C_1$.  But then \lemref{one-good} would imply that $u$ contains a face caressed by $C$, contradicting the assumption that $u$ is a bad node.  On the other hand, if $e$ began a chord path whose other endpoint was a vertex of $C_2$, then 
%%
%%
%%   The vertices in $N(u)$ are cyclically ordered around $u$ as $u_1,x_1,\ldots,x_r,u_2,y_1,\ldots,y_s$, where $u_1,u_2\in V(T_0)$, $x_1,\ldots,x_r,y_1,\ldots,y_s\in V(T_1)$, and $r,s\ge 2$.
%%
%%
%%If $u$ is not a face of $\dual{T}$, then $\dual{T}$ contains a path $P$ whose endpoints are on the boundary of $u$ and whose internal vertices are contained in the interior of $u$.  
%\end{proof}
%
%\begin{lem}\lemlabel{simple-neighbourhood}
%  If $u\in V(T_0)$ is a really bad node, then every node in $N(u)\cap V(T_1)$ has exactly one edge in common with $u$.
%\end{lem}
%
%\begin{proof}
%   Otherwise the two common edges would make an edge-cut of size 2. Since $\dual{T}$ is a 
%\end{proof}
%
%\begin{lem}
%   Suppose $T$ has maximum degree $\Delta$ and suppose that 
%  If $T$ has maximum degree $\Delta$ and $T_0$ and $T_1$ are such that $b_i < (1-\epsilon) n_i$ for each $i\in\{0,1\}$, then there exists a cycle $C'$ in $\dual{T}$ such that 
%\begin{enumerate}
%    \item $\tau' \ge \tau-\Delta$
%    \item $m_1'\ge m_1$
% \end{enumerate}
...
%
%\begin{proof}
%  Let $U\subseteq V(T_0)$ consist of all nodes $u\in V(T_0)$ such that $u$
%  is bad and every node in $N_{\dual{\bar{H}}}(u)$ is bad. In words, $u$
%  is bad and is entirely surrounded by bad nodes. Two of the bad nodes
%  surrounding $u$ are in $T_0$ and the remainder are in $T_1$.
%
%  We claim that $|U|\ge \kappa/(\Delta-1)$.  Indeed
%
%
%
%  Let $A$ be the subgraph of $\bar{H}$ induced by the following set $U$ of nodes: 
%  \begin{enumerate}
%    \item $U$ contains every bad node of $T_0$.
%    \item $U$ contains every bad node $u$ of $T_1$ such that $N_{A}(u)$ contains only bad nodes.
%  \end{enumerate}
%  By assumption, $U$ contains at least $b_0\ge (1-\epsilon)n_0$ nodes of $T_0$.  All these nodes have degree 2 in $T_0$.   
%
%
%\end{proof}
%
