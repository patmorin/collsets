...
For a planar graph $G$, we say that a set $S\subseteq V(G)$ is a
\emph{collinear set} if $G$ has a non-crossing straight-line drawing in
which the vertices of $S$ are all collinear.  


The \emph{dual} $\dual{G}$
...
to $\Omega(n^\alpha)$ for $\alpha>0$.  Jackson \cite{jackson:longest}
confirmed this bound with $\alpha=\log_2(1+\sqrt{5})-1 > 0.6942$.
Billinksi \etal\ \cite{bilinksi.jackson.ea:circumference} improved
this to the root of $4^{1/\alpha}-3^{1/\alpha}=2$, which implies
...
any planar graph of degree $n^{\alpha}$, with $\alpha < 0.075$.

Recently, Dumovi\'c \etal\ cite{dujmovic.goncalves.ea:every}, have shown
that every collinear set is \emph{free}. That is, for any any collinear
set $S$ in a planar graph $G$ and any set $X$ of $|S|$ points in $\R^2$,
there exists a non-crossing straight-line drawing of $G$ in which
the vertices of $S$ are drawn on the points of $X$.  Because of this,
collinear sets have a large number of applications in graph drawing and
related areas \cite{dujmovic:utility}.  The consequences of \corref{main} are discussed in \secref{discussion}.  
...
have the identity $\tau = \rho + \kappa$.
...
\begin{proof}
  ...
  by the 4-Colour Theorem \cite{robertson.seymour.ea:four-colour} $T[F]$ contains an independent
...
$1-O(1/\Delta^2)$ (\lemref{lots-of-really-i-bad}). \figref{few-caressed}
...
$X$ has at least $5\Delta+2$ nodes.
...
contains an edge of $C_b$.  It follows from the \lemref{bad-one-shared-edge}
...
\begin{figure}
   ...
   \figlabel{rrbad}
\end{figure}
We use the convention that $a_1,b_1\in N(x)$, $a_r,b_s\in N(y)$. We also
...
every (purple) edge contained in the red-blue boundary.
...
\end{figure}
     ...
     $T_0$ restores a path in $T_0$ from $y$ to $x$ (through $z_1$).
     ...
     and $w\cap C=w\cap C'$ for every $w\in N(v)$.  The nodes requiring special
     ...
     \begin{enumerate}
       ...
       and $z_1$.  The face $f$ must, however, contain the first and
       last edge of $P$.  All edges of $f$ are in the auxilliary graph
       $H$ and therefore $P$ contains an internal node of degree 3.
       But this contradicts the fact that $P$ is a keeper for $C$.

       Therefore $f$ contains $P$ on its boundary. Furthermore, $f$ is
       squeezed by $C'$, so $P$ is in the auxilliary graph $H'$ for $C'$.
    ...
    the fact that $x_{\Delta}-1,x_{\Delta},x_{\Delta+1}$ is a path of
     ...
     Furthermore, the nodes $y_0$ and $a_0$, which were bad nodes in $T_0$
     and $T_1$ are now leaves of $T_0'$ and $T_1'$.
...
In all four cases, the surgery on $C$ increases the number of leaves in
both $T_1$ and $T_2$.  Furthermore, the only edges of $C$ that are lost
in this operation are on the boundary of $u$.  Therefore, this operation
decreases the length of $C$ by at most $\Delta$.
