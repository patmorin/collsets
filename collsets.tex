...
\begin{proof}
  ...
  Next, $A_2$ contains degree-2 nodes of $T_0$ that are not bad.  If a node has degree-$2$ and contains no caressed face, then it is bad. Therefore each node in $A_2$ contains a caressed face. Therefore $|A_2|\le \kappa$, so $|A_1|+2|A_2|+3|A_1| <= 6\kappa$.  Picking up where we left off:

  \[
      \sum_{w\in A}\delta_w
       \le 6\kappa
     \le 6\alpha\ell/\Delta^2
     % & \text{(by assumption)}\\
    = O(\alpha n_0/\Delta)
  ...
  \begin{align*}
   \sum_{w\in A'}\Delta \tau_w
    ...
    & < 2\Delta\kappa + 12\Delta\kappa & \text{(by defining $A'_1$, $A'_2$, $A'_{\ge 3}$ and arguing as above)}\\
    & = 14\Delta\kappa \\
