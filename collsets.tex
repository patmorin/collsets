...
\begin{figure}
\begin{center}
	  \includegraphics{figs/touched-pinched-caressed}
\end{center}
...
\begin{proof}
	...
	\begin{figure}
		...
		\begin{tabular}{cc}
			...
			\includegraphics{figs/discharge-A-5} \\
			 2.c & 3.a \\[1.5em]
			 \multicolumn{2}{c}{\includegraphics{figs/discharge-A-6} } \\
			\multicolumn{2}{c}{3.b}
		\end{tabular}
		\end{center}
  ...
  \end{enumerate}
  \item $R$ contains at least one pinched face, but no pinched face in $R$ shares an edge with $P$.  In this case, consider the face $g$ of $H$ that is contained in $R$ and has $P$ on its boundary. There are one or more pinched faces $f_1,\ldots,f_k$, where each $f_i$ shares a chord path $P_i$ with $f$. There are two cases to consider:
	  \begin{enumerate}
		  \item If $P_1$ is a keeper path, then $f_1$ is not contained in $u$, and we assign all of $P$'s charge to $P_1$.
		  \item If $P_1$ is not a keeper path, then there is a pinched face $f_1'$ in $R$ that shares at least one edge with $P_1$.  The face $f_1$ contains one or more chord paths $P_{1,1},\ldots,P_{1,k}$ that are each disjoint from $P_1$.  The face $f_1'$ contains one or more chord paths $P_{1,1}',\ldots,P_{1,\ell}'$ that are each disjoint from $P_1$.  We will recurse on each of these chord paths, but before doing so we move the charge from $P$ to $P_{1,1}$ and we move the charge from $f$ and $f'$ to $P_{1,1}'$.
	  \end{enumerate}
\end{enumerate}
