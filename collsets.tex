...
Recently, Dumovi\'c \etal\ \cite{dujmovic.frati.ea:every}, have
...
whose dual is $T^*$, $v(T)=\Theta(c(\dual{T})/\Delta^4)$ by demonstrating a
...
\begin{proof}
  ...
  of $T$) that we will also call $C$.  We will perform local modifications 
  on $C$ so that it contains all the vertices in $F'$.
...
\begin{lem}\lemlabel{one-caressed}
   ...
   of $P\cup C$ that have $P$ on their boundary. Then $R$
