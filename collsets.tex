\documentclass[english]{socg-lipics-v2018}
...
\titlerunning{Dual Circumference and Collinear Sets}
\authorrunning{V. Dujmovi\'c and P. Morin}

\author{Vida Dujmovi\'c}{Department of Computer Science and Electrical Engineering, University of Ottawa}{vida.dujmovic@uottawa.ca}{https://orcid.org/0000-0001-7250-0600}{Partly funded by NSERC and the Government of Ontario.}

\author{Pat Morin}{School of Computer Science, Carleton University}{morin@scs.carleton.ca}{https://orcid.org/0000-0003-0471-4118}{Partly funded by NSERC.}

\Copyright{Vida Dujmovi\'c and Pat Morin}

\subjclass{Human-centered computing~Graph drawings}
...
\keywords{Collinear Sets, Free Sets, Graph Circumference, Tait's Conjecture}
...
\begin{abstract}
   ...
   has a plane straight-line drawing in which some \emph{collinear set}
   of $\Omega(\ell/\Delta^4)$ vertices lie on a line.  Using the current
   lower bounds on the length of longest cycles in 3-regular 3-connected
   graphs, this implies that $T$ has a collinear set of
   size $\Omega(n^{0.8}/\Delta^4)$.  Very recently, Dujmovi\'c
   \etal\ (SODA 2019) showed that, if $S$ is a collinear set in a
   triangulation $T$ then, for any point set $X\subset\R^2$ with
   $|X|=|S|$, $T$ has a plane straight-line drawing in which the
   vertices of $S$ are drawn on the points in $X$.  Because of this,
   collinear sets have numerous applications in graph drawing and related areas.
\end{abstract}
...
$S$ are all collinear.  The \emph{dual} $\dual{G}$ of a plane graph $G$
is the graph whose vertex set $V(\dual{G})$ is the set of faces in $G$
and in which $f,g\in E(\dual{G})$ if and only if the face $f$ and the face
$g$ have at least one edge in common.  The \emph{circumference}, $c(G)$,
of a graph $G$ is the length of its longest cycle. In \secref{proof},
...
\cite{liu.yu.zhang:circumference} who show that $\alpha>0.8$.  Together
with \thmref{main}, this result implies the following corollary:
...
It is known \cite{bose.dujmovic.ea:polynomial} that every planar graph $G$
has a collinear set of size $\Omega(\sqrt{n})$. \corref{main} therefore
improves on this bound for bounded degree planar graphs and, indeed for
the family of $n$-vertex planar graphs of maximum degree 
$\Delta\in O(n^{\alpha})$, with $\alpha < 0.075$.

Recently, Dujmovi\'c \etal\ \cite{dujmovic.frati.ea:every},
have shown that every collinear set is \emph{free}. That is,
for any planar graph $G$, any collinear set $S\subseteq V(G)$,
and any set $X\subset\R^2$, with $|X|=|S|$, there exists a
non-crossing straight-line drawing of $G$ in which the vertices of
$S$ are drawn on the points of $X$.  Because of this, collinear
sets have immediate applications in graph drawing and related areas.  
For applications of \corref{main}, including untangling
...
column planarity~\cite{barba.evans.ea:column,evans.kusters.ea:column,dalozzo.dujmovic.ea:drawing,dujmovic:utility},
universal point
subsets~\cite{digiacomo.liotta.ea:how,angelini.binucci.ea:universal,dalozzo.dujmovic.ea:drawing,dujmovic:utility}, and partial
simultaneous geometric drawings~\cite{evans.kusters.ea:column,barba.hoffmann.ea:column,angelini.evans.ea:sefe,blasius.kobourov.ea:simultaneous,dujmovic:utility} the reader is
referred to Dujmovi\'c \cite{dujmovic:utility}.
\corref{main}, gives improved bounds for all of these problems for planar graphs of maximum $\Delta\in o(n^{0.075})$.


%The consequences of \corref{main}
%are discussed in \secref{discussion}.
...
whose dual is $\dual{T}$, $v(T)=\Theta(c(\dual{T})/\Delta^4)$ by demonstrating a
relationship between proper good curves in $T$ and cycles in $\dual{T}$.

As shown by Ravsky and Verbitsky
\cite{ravsky.verbitsky:on,ravsky.verbitsky:on-arxiv}, the inequality
$v(T) \le c(\dual{T})$ is easy: If $T$ is a triangulation that has a
proper good curve $C$ containing $k$ vertices, then a slight deformation
of $C$ produces a proper good curve that contains no vertices. This
curve intersects a cyclic sequence of faces $f_0,\ldots,f_{k'-1}$ of
$T$ with $k'\ge k$.  In this sequence, $f_i$ and $f_{(i+1)\bmod k'}$
share an edge, for every $i\in\{0,\ldots,k'-1\}$, so this sequence is
a closed walk in the dual $\dual{T}$ of $T$.  Property~1 of good curves
...
\end{center}
	  \caption{Faces of $\dual{T}$ that are pinched and caressed by $C$. $C$ is bold, caressed faces are blue, pinched faces are pink, and untouched faces are unshaded.}
...
\begin{proof}
  ...
  by $F$.  The graph $T[F]$ is planar and has $k$ vertices. Therefore,
...
all paths that are special with respect to some pinched face $f$ of $\dual{T}$.
...
We will repeatedly make use of the following well-known property of
...
single node $u$ of $T_0$ that contains $\kappa_u=0$ caressed faces and
...
\begin{proof}
   ...
   and has at least 2 chord paths $P_1,\ldots,P_r$ on its boundary.
  ...
  \begin{enumerate}
	...
	of $P$ and each $X_i$ has a chord path $P_i$ path in common
...
The proof of \lemref{common-face} is deferred to \appref{missing-proofs}.
...
The proof of \lemref{lots-of-really-bad} is deferred to \appref{missing-proofs}.
...
contains an edge of $C_b$.  It follows from \lemref{bad-one-shared-edge}
that among any sequence of $\Delta$ consecutive nodes in $X$, at least
one node has $|N(u)|\ge 5$ because $r\ge 2$.  Let $u$ be any such node
that is not among the first $2\Delta$ or last $2\Delta$ nodes of $X$.
Such a $u$ always exists because $X$ contains at least $5\Delta$ nodes.
...
\begin{figure}
   ...
   \end{center}
   \caption{A carefully selected really really bad node $u$.}
   \figlabel{rrbad}
\end{figure}
We use the convention that $a_1,b_1\in N(x_1)$, $a_r,b_s\in N(y_1)$. We also
...
\section{Proofs of Lemmas \ref{lem:common-face} and \ref{lem:lots-of-really-bad}}
\applabel{missing-proofs}

\begin{proof}[Proof of \lemref{common-face}]
   First observe that, since $u$ has degree 2, $C\cap u$ has exactly
   two connected components $C_1$ and and $C_2$. Thus $u$'s boundary
   consists of $C_1$, $C_2$ and two chord paths $P_1$ and $P_2$.
...
\begin{proof}[Proof of \lemref{lots-of-really-bad}]
  First consider the tree $T_0$.  From \obsref{degree-touched},
  we know that $\tau_0\ge \ell/\Delta$.  By assumption,
  $\kappa_0\le\kappa\le \alpha\ell/\Delta^2 < \tau_0/6$
  so, by \lemref{few-caressed-implies-many-nodes}, $n_0 \ge
