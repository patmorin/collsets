...
\begin{proof}
   ...
   boundary of $u$.  There are a few cases to rule out (see \figref{common_face}):
   \begin{figure}
       \begin{center}
           \centering{
           \begin{tabular}{cc}
...
\begin{proof}
   By \lemref{common_face}, there is one face $f$ of $\dual{T}$ contained in $u$ that contains both the first and last edge of $P$ and there is one face $g$ of $\dual{T}$ contained in $w$ that contains both the first and last edge of $P$.  By \lemref{one_shared_edge}, $f$ and $g$ have only one edge in common so the first and last edge of $P$ must be the same, i.e., $P$ consists of a single edge.
\end{proof}
...
\begin{proof}
    A node $u$ is a \emph{fringe node} if it is really bad but not really really bad. A node $u$ is a \emph{critical node} if it is bad but not really bad.  Observe that every fringe node $u$ is in $N(w)$ for some critical node $w$. To bound the number of fringe nodes, it therefore suffices to bound $\sum_{w}|N(w)|$ where the sum is over all critical nodes.

    By \lemref{common_face}, for each critical node $w$ there is a single face $f$ of $\dual{T}$ whose boundary contains $C\cap w$. Since $w$ is bad, $C\cap w$ contains exactly two components and therefore at least two edges of $f$ are not contained in $C\cap w$.  Therefore the number of edges of $\dual{T}$ in $C\cap w$ is at most $\Delta-2$.  Therefore $N(w)$ contains exactly two nodes of $T_i$ and at most $\Delta-2$ nodes of $T_{1-i}$.  Therefore $|N(w)|\le \Delta$ for each critical node $w$.
...
We summarize the situation so far.  By \lemref{cycle_to_curve}, finding a large collinear set is equivalent to finding a cycle in $\dual{T}$ that caresses many faces.  By existing results on the circumference of cubic triconnected graphs, $\dual{T}$ has a cycle $C_0$ of length $\ell=\Omega(n^{\alpha})$ for some $\alpha > 0.8$.  Thus we assume that $\dual{T}$ has a cycle $C_0$ of length $\ell$ and we want to show the existence of a cycle $C$ that caresses $\Omega(\ell/\Delta^4)$ faces.

Because each face of $\dual{T}$ has at most $\Delta$ edges, $C_0$ touches $\Omega(\ell/\Delta)$ faces (\obsref).  To complete the proof of \thmref{main} we must deal with the situation where $C_0$ caresses $o(\ell/\Delta^4)$ faces and therefore each of $T_0$ and $T_1$ has $o(\ell/\Delta^4)$ leaves (\lemref{one_caressed_leaf}), $\Omega(\ell/\Delta)$ nodes (\lemref{few_caressed_implies_many_nodes}), and the fraction of really really bad nodes in $T_0$ and $T_1$ is $1-O(1/\Delta^2)$ (\lemref{lots_of_really_really_bad}).  \figref{few_caressed_ii} illustrates an extreme example of this.  To handle cases like these, the only option is to perform surgery on the cycle $C$ to increase the number of caressed faces. To complete the proof we will show that such a surgery is always possible.
...
$k=\sum_{u\in S}(\deg_{T_0}(u)-1)$.
Furthermore,
\[
    O(\epsilon n_0/\Delta) \ge |L| \ge \sum_{u\in S}(\deg_{T_0}(u)-2)
     = |S| + \sum_{u\in S}(\deg_{T_0}(u)-1) = |S| + k \enspace .
\]
...
\lemref{bad_one_shared_edge} that among any sequence of $\Delta$
