...
\author{Vida Dujmovi\'c\thanks{School of Computer Science and Electrical Engineering, University of Ottawa}\,\, and 
...
\begin{abstract}
   ...
   size $\Omega(n^{0.8}/\Delta^4)$.  Very recently, Dujmovi\'c \etal\
   (SODA 2019) showed that, if $S$ is a collinear set in a triangulation
   $T$ then, for any point set $X\subset\R^2$ with $|X|=|S|$, $T$ has
   a non-crossing straight-line drawing in which the vertices of $S$
   are drawn on the points in $X$.  Because of this, collinear sets have
   numerous applications in graph drawing and related areas.
\end{abstract}

\section{Introduction}

Throughout this paper, all graphs are simple and finite.  For a planar
graph $G$, we say that a set $S\subseteq V(G)$ is a \emph{collinear set}
if $G$ has a non-crossing straight-line drawing in which the vertices of
$S$ are all collinear.  A \emph{plane graph} is a planar graph $G$ along
...
We say that a cycle $C$ in a plane graph is \emph{non-trivial} if it
...
Our strategy is to define a tree structure, $T_0$ on groups of
faces contained in the interior of $C$ and a similar structure, $T_1$ on
groups of faces in the exterior of $C$.  We will then show that every leaf
of $T_0$ or $T_1$ contains a face caressed by $C$. In \figref{few-caressed},
...
if $C$ does not caress many faces, then it is possible to perform a
surgery on $C$ that increases the number of leaves in $T_0$ and $T_1$.
Performing this surgery often enough will ensure that $T_0$ and $T_1$
have many leaves and therefore $C$ caresses many faces.
...
of leaves in both $T_0$ and $T_1$.  Before finishing up, we need a final
...
contains an edge of $C_b$. [At this point we must take some extra care
because it is possible that $a_i=b_j$ for some values of $i$ and $j$.]
It follows from \lemref{bad-one-shared-edge} that among any sequence of
$\Delta$ consecutive nodes in $X$, at least one node has $|N(u)|\ge 5$
because $r\ge 2$.  Let $u$ be any such node that is not among the first
$2\Delta$ or last $2\Delta$ nodes of $X$.  Such a $u$ always exists
because $X$ contains at least $5\Delta$ nodes.

$N(u)$ contains at least 5 nodes (refer to \figref{rrbad}):
\begin{enumerate}
   \item the really really bad neighbours $x_1$ and $y_1$ of $u$ in $T_0$;
   \item a path $a_1,\ldots,a_r$, $r\ge 2$, of really bad nodes in $T_1$, each sharing
     an edge of $C_a$ with $u$;
   \item a path $b_1,\ldots,b_s$, $s\ge 1$, of really bad nodes in $T_1$, each sharing an edge of $C_b$ with $u$.
\end{enumerate}
\begin{figure}
   \begin{center}
     \includegraphics{figs/second-try-1}
   \end{center}
   \caption{A carefully selected really really bad node $u$.  Note,
    this figure is potentially misleading, we may have $a_i=b_j$ for
    some values of $i$ and $j$.}
   \figlabel{rrbad}
\end{figure}
We use the convention that $a_1,b_1\in N(x_1)$, $a_r,b_s\in N(y_1)$. 

The ``surgery'' we will perform focuses on the nodes $u$ and $a_1$.
Consider the two components of $C\cap a_1$. One of these components shares
an edge with $u$ and the other.  By \lemref{bad-one-shared-edge}, the
other component, $q$, does not share an edge with $u$.  Imagine removing
$u$ from $T_0$, thereby separating $T_0$ into a component $T_x$ containing
$x_1$ and a component $T_y$ containing $y_1$.  Equivalently, one can
think of removing the edges of $u$ from $C$ separating $C$ into two
paths $C_x$ and $C_y$ on the boundary of $T_x$ and $T_y$, respectively.
We distinguish between two major cases (see \figref{main-cases}):

\begin{figure}
   \begin{center}
     \begin{tabular}{cc}
       \includegraphics{figs/final-cases-1} &
       \includegraphics{figs/final-cases-2} \\
       (1) & (2)
     \end{tabular}
   \end{center}
   \caption{The cases in the proof of \thmref{main}.}
   \figlabel{main-cases}
\end{figure}
\begin{enumerate}
   \item $q\subset C_x$.  In this case, we punt. By
   \lemref{one-edge-between} $a_1-C$ consists of two edges and exactly
   one of these edges, $e$, is not incident to $u$. Since $x_1$ is the
   endpoints of a path $x_1,\ldots,x_{2\Delta}$ really really bad nodes in
   $T_x$, $e$ is incident to some really really bad node $x_i$ of $T_0$,
   for some $i<\Delta$.  Define $x_0=u$.  At this point, we let $x_i$
   take the role of $u$, $x_{i+1}$ take the role of $y_1$ and $x_{i-1}$
   take on the role of $x_1$.  With this change of variables, we find ourselves
   in Case~2, described below.

   \item $q\subset C_y$.  At this point, consider the component
   $p$ of $C\cap a_0$ that contains an edge of $u$.  Let
   $i\in\{1,\ldots,\Delta\}$ be the maximum value such that $p$ contains
   at least one edge of $x_i$. (This is the same value of $i$ defined
   in Case~1, above.)  Note that this implies $p$ contains one
   of the two components of $C\cap x_j$ for each $j\in\{1,\ldots,i-1\}$.
   ...
   $T_0'$ and $T_1'$.  The relationship between $(T_0,T_1)$ and $(T_0',T_1')$
   ...
   Refer to \figref{recolouring}.
   For our modification, we recolour $u,x_1,\ldots,x_{i-1}$ blue and
   recolour $a_1$ red.  Observe that, because $q\subset C_y$ and $p\cap
   C_x\ne\emptyset$, this implies that the red set is simply connected
   and its boundary $C'$ is a simple cycle.  The new trees $T_0'$ and
   $T_1'$ are therefore well defined.  We now make two claims that will complete our proof.

   \begin{figure}
     \begin{center}
       \begin{tabular}{cc}
         \includegraphics{figs/final-cases-3} &
         \includegraphics{figs/final-cases-4} 
       \end{tabular}{cc}
     \end{center}
   ...
   \begin{clm}\clmlabel{a1-caressed}
      The face $a_0$ is caressed by $C'$.
   \end{clm}

   To see why these two claims complete the proof observe that, together,
   ...
   This surgery recolours at most $\Delta+2$ nodes of $T_0$
   and $T_1$, so the difference in length between $C$ and $C'$
   is at most $\Delta^2+2\Delta$.  If we start with a cycle $C$
   of length $\ell$, then we can perform this recolouring at least
   $\ell/(4\Delta^2+4\Delta)$ times before the length of $C$ decreases
   ...
   $\ell/(4\Delta^2+4\Delta)\in\Omega(\ell/\Delta^2)\subset\Omega(\ell/\Delta^4)$
   and we are also done.

   Thus, all that remains is to prove \clmref{only-bad} and
   \clmref{a1-caressed}.

   To prove \clmref{only-bad} we observe that $C$ and $C'$ differ only
   on the boundaries of nodes that are recoloured.  The nodes $u$ and
   $x_1,\ldots,x_i$ are all really really bad. Therefore the nodes
   of $T_1$ that share a boundary with these nodes are really bad.
   Furthermore, $x_{i+1}$ and $y_1$ are really really bad.  Thus, the
   changes caused by recolouring $u,x_1,\ldots,x_{i-i}$ do not violate
   the conditions of the lemma.

   The node $a_1$ is really bad and its two neighbours $a_0$ and $a_2$
   in $T_1$ are also really because each of them shares at least one
   edge with a really really bad node in $T_0$:  $a_0$ shares an edge
   with $x_i$ and $a_2$ shares an edge with $u$.  Finally, since $a_1$
   is really bad, the nodes of $T_0$ that share share an edge of $C$
   with $a_1$ are bad.  Thus, all the nodes whose intersection with
   $C$ and $C'$ are different are bad nodes. This proves \clmref{only-bad}.


   To prove \clmref{a1-caressed} we look consider the boundary of $a_0$
   after the recolouring operation.  After recolouring, this boundary
   consists of, in cyclic order:
   \begin{enumerate}
     \item  An edge shared between $a_0$ and $a_1$.  This edge is in $C'$ since
 $a_0$ is in $T_1'$ and $a_1$ is in $T_0'$.  

     \item A path of edges shared with nodes in
     $x_{i},\ldots,x_{i+\Delta}$. By the choice of $u$, these are (really
     really bad) nodes that are in $T_0$. Therefore, this part of the
     bounary of $a_0$ is contained in $C'$.

     \item An edge shared between $a_0$ and another node $a_{-1}$. The
     node $a_{i-1}$ shares an edge with some really really bad node in
     $x_{i},\ldots,x_{i+\Delta}$. Therefore $a_{i-1}$ is (really) bad,
     it is in $T_1$ and $T_1'$ and therefore this edge is not contained in $C'$.

     \item A path of edges that contains at least one edge of $C_y$. If
     we fix an embedding in which the outer face it some face of $T_1$
     ...
     path does not contain any edges of $u,x_1,\ldots,x_{i-1}$ that
     ...
     $u,\ldots,x_{i-1}$ followed by a sequence of edges from $C_y$. The
